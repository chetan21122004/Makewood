...
const AboutUs = () => {
  ...
  return (
    <section className="w-full py-6 sm:py-4 bg-white" id="about">
      <div className="container px-4 sm:px-6 lg:px-8 mx-auto">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Vision, Mission, Values - Modern Stats Design */}
          <div className="mb-20">
            {/* Section Header */}
            <div className="text-center mb-16">
              <div className="inline-block mb-4">
                <div className="flex items-center justify-center space-x-2  font-semibold text-[#7A2E1D] bg-[#7A2E1D]/5 rounded-full py-2 px-4">
                  <span className="w-2 h-2 rounded-full bg-[#7A2E1D]"></span>
                  <span>Our Core Values</span>
                </div>
              </div>
          
              <p className="text-gray-600 max-w-2xl mx-auto text-lg leading-relaxed">
                Three foundational pillars that shape our approach to creating exceptional spaces
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6 lg:gap-10">
              {visionCards.map((card, index) => (
                <div key={card.title}
                  className="group bg-white rounded-2xl p-8 shadow-sm hover:shadow-xl transition-all duration-500 border border-gray-100 relative overflow-hidden animate-on-scroll"
                >
                  {/* Top Accent Line */}
                  <div className="absolute top-0 left-0 right-0 h-1 bg-[#7A2E1D]/20 group-hover:bg-[#7A2E1D] transition-colors duration-500"></div>

                  {/* Content */}
                  <div className="relative">
                    {/* Icon with Number */}
                    <div className="flex items-center space-x-4 mb-6">
                      <div className="w-12 h-12 rounded-xl bg-[#7A2E1D]/5 flex items-center justify-center group-hover:bg-[#7A2E1D]/10 transition-colors duration-500">
                        <card.icon className="w-6 h-6 text-[#7A2E1D]" />
                      </div>
                      <div className="text-4xl font-bold text-[#7A2E1D]/20 group-hover:text-[#7A2E1D] transition-colors duration-500">
                        {String(index + 1).padStart(2, '0')}
                      </div>
                    </div>

                    {/* Title & Description */}
                    <h4 className="text-xl font-bold mb-4 text-gray-900 group-hover:text-[#7A2E1D] transition-colors duration-300">
                      {card.title}
                    </h4>
